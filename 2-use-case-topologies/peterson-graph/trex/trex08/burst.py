...
class STLS1(object):
    ...
    def create_stream (self):
        ...
        base_pkt4 =  Ether()/IP(src="10.101.8.108",dst="10.101.6.106")/UDP(dport=12,sport=1025)
        base_pkt5 =  Ether()/IP(src="10.101.8.108",dst="10.101.7.107")/UDP(dport=12,sport=1025)
        base_pkt6 =  Ether()/IP(src="10.101.8.108",dst="10.101.2.102")/UDP(dport=12,sport=1025)
        ...
        return STLProfile( [ STLStream( isg = 10.0, # start in delay 
                             ...
                             STLStream( self_start = False, # Stream is disabled. Will run because it is pointed from S1
                                        ...
                                        mode = STLTXSingleBurst( pps = 1000, total_pkts = self.burst_size ),
                                        next    = 'S4' ),

                             STLStream( self_start = False, # Stream is disabled. Will run because it is pointed from S1
                                        name   ='S4',
                                        packet = STLPktBuilder(pkt = base_pkt4/pad),
                                        mode = STLTXSingleBurst( pps = 9000, total_pkts = self.burst_size ),
                                        next    = 'S5' ),

                             STLStream( self_start = False, # Stream is disabled. Will run because it is pointed from S1
                                        name   ='S5',
                                        packet = STLPktBuilder(pkt = base_pkt5/pad),
                                        mode = STLTXSingleBurst( pps = 11000, total_pkts = self.burst_size ),
                                        next    = 'S6' ),
                                        
                             STLStream(  self_start = False, # Stream is disabled. Will run because it is pointed from S2
                                         name   ='S6',
                                         packet = STLPktBuilder(pkt = base_pkt6/pad),
